...
const HistoryPage = () => {
  ...
  const confirmDelete = async () => {
    ...
    if (deleteConfirmationName !== reportToDelete.name) {
      setDeleteError('Report name does not match. Please enter the exact name.');
      return;
    }
    ...
    try {
      await api.post('/api/delete-report/', {
      ...
      setDeleteError(err.response?.data?.error || 'Failed to delete report. Please try again.');
    } finally {
  ...
  const saveReport = async () => {
    ...
    try {
      ...
      // Add the new report to the saved reports list if it matches search
      if (saveName.trim().toLowerCase() === searchTerm.toLowerCase()) {
      ...
      // If no search term, set search term to saved name and search
      if (!searchTerm) {
        setSearchTerm(saveName.trim());
        setTimeout(() => {
          // Trigger search after state update
          api.get('/api/report/by-name/', { params: { name: saveName.trim() } })
            .then(res => {
  ...
  return (
    <div className="min-h-screen container mx-auto px-4 sm:px-6 py-8">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <h1 className="text-3xl md:text-4xl font-bold mb-4">Saved Reports</h1>
          <p className="text-muted-foreground text-lg">
            Search and view your saved analysis reports
          </p>
        </motion.div>
                ...
                {hasSearched && savedReports.length === 0 ? (
                   <div className="text-center py-12">
                    ...
                    <p className="text-muted-foreground mb-6">
                      No saved reports found with the name "{searchTerm}".
                    </p>
                    <button
                        ...
                        <div className="flex items-center justify-between pt-4 border-t">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 text-sm text-red-500 hover:underline ml-3"
                            >
        ...
        {showDeleteModal && (
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <motion.div
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              className="bg-background rounded-2xl p-6 w-full max-w-md border-2 border-red-500/20"
            >
              ...
              <p className="text-muted-foreground mb-4">
                This action cannot be undone. To confirm deletion, please type the report name 
                <span className="font-bold text-foreground"> {reportToDelete?.name} </span> 
                below:
              </p>
              ...
              {deleteError && (
                <div className="text-red-500 text-sm mb-4 bg-red-500/10 p-2 rounded">{deleteError}</div>
              )}
